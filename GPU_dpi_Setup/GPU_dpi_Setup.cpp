...
// Product    GPU-dpi
// File       GPU_dpi_Setup/OpenNet_Setup.cpp
...
static unsigned int DisplayMenu (GPU_dpi::SetupTool * aSetupTool);
static void         DisplayTitle(const char * aTitle, bool aDebug);

static unsigned int ReadAnswer(unsigned int aCount);

// ===== Mode ===============================================================
static int Install   (GPU_dpi::SetupTool * aSetupTool);
static int Interactif(GPU_dpi::SetupTool * aSetupTool);
static int Uninstall (GPU_dpi::SetupTool * aSetupTool);
static int Wizard    (GPU_dpi::SetupTool * aSetupTool);

// ===== Wizard =============================================================
static int          Wizard_Page(GPU_dpi::SetupTool * aSetupTool, unsigned int * aPage);
...
int main(int aCount, const char * * aVector)
{
    ...
    KMS_TOOL_BANNER("GPU-dpi", "GPU_dpi_Setup", VERSION_STR, VERSION_TYPE);

    if (!OSDep_IsAdministrator())
    {
        fprintf(stderr, "USER ERROR  GPU_dpi_Setup must be run as an administrator\n");
        return __LINE__;
    }
    ...
    GPU_dpi::SetupTool * lSetupTool = GPU_dpi::SetupTool::Create(lDebug);
...
unsigned int DisplayMenu(GPU_dpi::SetupTool * aSetupTool)
{
...
int Install(GPU_dpi::SetupTool * aSetupTool)
{
    assert(NULL != aSetupTool);

    printf("Installation ...\n");

    GPU_dpi::Status lStatus = aSetupTool->Install();
    assert(GPU_dpi::STATUS_OK == lStatus);

    return Wizard(aSetupTool);
}

int Interactif(GPU_dpi::SetupTool * aSetupTool)
{
    assert(NULL != aSetupTool);
    ...
    for (;;)
    {
        ...
        {
            GPU_dpi::Status lStatus = aSetupTool->Interactif_ExecuteCommand(lAnswer);
            switch (lStatus)
            {
            case GPU_dpi::STATUS_OK: break;

            case GPU_dpi::STATUS_REBOOT_REQUIRED:
...
int Uninstall(GPU_dpi::SetupTool * aSetupTool)
{
    assert(NULL != aSetupTool);

    GPU_dpi::Status lStatus = aSetupTool->Uninstall();
    switch (lStatus)
    {
    case GPU_dpi::STATUS_OK: break;
...
int Wizard(GPU_dpi::SetupTool * aSetupTool)
{
    assert(NULL != aSetupTool);
...
int Wizard_Page(GPU_dpi::SetupTool * aSetupTool, unsigned int * aPage)
{
    assert(NULL != aSetupTool);
    assert(NULL != aPage     );
    ...
    GPU_dpi::Status lStatus = aSetupTool->Wizard_ExecutePage(aPage, lAnswer);
    switch (lStatus)
    {
    case GPU_dpi::STATUS_OK: break;
