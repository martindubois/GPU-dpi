...
scp Binaries/GPU-dpi_Run $TEST_COMPUTER:$DST_FOLDER/Binaries
if [ 0 != $? ] ; then
    echo ERROR  scp Binaries/GPU-dpi_Run $TEST_COMPUTER:$DST_FOLDER/Binaries  failed
    RESULT=20
fi
...
scp GPU-dpi_Run/Tests/Test03.txt $TEST_COMPUTER:$DST_FOLDER/GPU-dpi_Run/Tests
if [ 0 != $? ] ; then
    echo ERROR  scp GPU-dpi_Run/Tests/Test00.txt $TEST_COMPUTER:$DST_FOLDER/GPU-dpi_Run/Tests  failed
    RESULT=50
fi
