...
OPEN_NET_FUNCTION_DECLARE( FilterFunction )
{
    OPEN_NET_FUNCTION_BEGIN
        ...
        int lR;
        lR = 0;
        if ( ( 0 != lIP ) && ( TCP_IP_PROTOCOL == lIP_Protocol ) )
        {
            OPEN_NET_GLOBAL unsigned char * lData = TCP_Data( lIP );
            unsigned int lDataSize_byte = lIP_DataSize_byte - TCP_HEADER_SIZE_byte;
            if ( (                           16384 - 54 ) < lDataSize_byte ) { lDataSize_byte = (                           16384 - 54 ); }
            if ( ( aBufferHeader->mPacketSize_byte - 54 ) < lDataSize_byte ) { lDataSize_byte = ( aBufferHeader->mPacketSize_byte - 54 ); }
            unsigned char lA_Counters[ 6 ];
            RegEx lA_RegEx;
            OPEN_NET_CONSTANT RegEx_State lA_States[] =
            {
                { REG_EX_DIGIT, 3,   3, 0x0000 },
                { '-', 1,   1, 0x0000 },
                { REG_EX_DIGIT, 3,   3, 0x0000 },
                { '-', 1,   1, 0x0000 },
                { REG_EX_DIGIT, 4,   4, 0x0000 },
